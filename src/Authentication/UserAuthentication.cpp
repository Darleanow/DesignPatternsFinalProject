 ...
 * @brief Implements the User and AuthenticationService classes.
 */

#include "TechMa/Authentication/UserAuthentication.h"

User::User(int id, std::string username, std::string password_hash, UserRole role)
    : m_id(id), m_username(std::move(username)), m_password_hash(std::move(password_hash)), m_role(role) {}

int User::id() const { return m_id; }

const std::string& User::username() const { return m_username; }

const std::string& User::password_hash() const { return m_password_hash; }
...
AuthenticationService::AuthenticationService() {
  register_user(std::make_shared<User>(1, "admin", hash_password("admin123"), UserRole::ADMIN));
  register_user(std::make_shared<User>(2, "tech", hash_password("tech123"), UserRole::TECHNICIAN));
  register_user(std::make_shared<User>(3, "guest", hash_password("guest123"), UserRole::GUEST));
}
