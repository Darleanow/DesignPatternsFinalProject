...
void CreateInterventionDialog::setup_ui()
{
    ...
    for (auto e : {
        ...
        m_expertise_input->addItem(
            QString::fromStdString(to_string(e)),
            static_cast<int>(e)
        );
    }
...
void CreateInterventionDialog::setup_connects()
{
    ...
    setup_combo(m_expertise_input, [this] {
        update_ok_state();
    });
    setup_combo(m_complexity_input, [this] {
        update_ok_state();
    });
    setup_combo(m_technician_input, [this] {
        update_ok_state();
    });

    connect(m_date_input, &QDateEdit::dateChanged, this,
            &CreateInterventionDialog::update_ok_state);
    connect(m_duration_input, &QSpinBox::valueChanged, this,
            &CreateInterventionDialog::update_ok_state);
    connect(m_location_input, &QLineEdit::textChanged, this,
            &CreateInterventionDialog::update_ok_state);
}
